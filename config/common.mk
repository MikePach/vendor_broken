...
# Set broken version
ifdef BROKEN_RELEASE
    BROKEN_VERSION := Broken-$(PRODUCT_VERSION_MAINTENANCE).$(BROKEN_VERSION_MAJOR).$(BROKEN_VERSION_MINOR)-$(shell date +%Y%m%d-%H%M)
else
    BROKEN_VERSION := Broken-$(BROKEN_VERSION_MAJOR).$(BROKEN_BUILDTYPE)-$(shell date +%Y%m%d-%H%M)
endif
